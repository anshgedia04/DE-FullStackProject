...
import { FaTruck, FaShippingFast, FaSearch, FaSort, FaClock } from 'react-icons/fa';
...
function AdminDispatch() {
  ...
  useEffect(() => {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        case 'newest':
          return new Date(b.dispatchedAt || b.createdAt) - new Date(a.dispatchedAt || a.createdAt);
        case 'oldest':
          return new Date(a.dispatchedAt || a.createdAt) - new Date(b.dispatchedAt || b.createdAt);
        case 'amount-high':
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex justify-center items-center">
        <div className="text-center">
          <Spinner type="diamond" text="Loading dispatched orders..." />
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex flex-col justify-center items-center px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-600 mb-8">
            No orders are currently in transit. Orders will appear here once they've been dispatched to customers.
          </p>
        </div>
      </div>
    );
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center space-x-4 text-sm text-gray-500">
              <span className="flex items-center space-x-2 bg-purple-100 px-3 py-2 rounded-full">
                <FaShippingFast className="text-purple-600" />
                <span className="font-medium text-purple-800">{filteredOrders.length} orders in transit</span>
              </span>
            </div>
          </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <div className="relative">
                  <select
                    ...
                    className="appearance-none bg-white border-2 border-gray-200 rounded-xl px-4 py-3 pr-10 text-gray-700 font-medium focus:border-purple-400 focus:ring-2 focus:ring-purple-100 focus:outline-none transition-all duration-300 cursor-pointer hover:border-gray-300 shadow-sm min-w-[180px]"
                  >
                    <option value="newest">Recently Dispatched</option>
                    <option value="oldest">Oldest Dispatched</option>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">
                    {searchTerm ? 'Filtered Results' : 'In Transit'}
                  </p>
            ...
            <div className="card-luxury p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">
                    {searchTerm ? 'Express in Results' : 'Express Orders'}
                  </p>
                  <p className="text-2xl font-bold text-gray-900">
                    {filteredOrders.filter(order => order.shippingType === 'express').length || 0}
                  </p>
                  {searchTerm && (
                    <p className="text-xs text-gray-500 mt-1">
                      of {orderGroup.filter(order => order.shippingType === 'express').length || 0} total express
                    </p>
                  )}
                </div>
          ...
          <div className="grid gap-6 grid-cols-1 lg:grid-cols-2">
            {filteredOrders.map((order) => (
              <div key={order.id || order._id} className="animate-fade-in">
                <SingleAdminItem
                  order={order}
                  onDelete={fetchDispatchData}
                  onStatusChange={fetchDispatchData}
                />
              </div>
            ))}
          </div>
